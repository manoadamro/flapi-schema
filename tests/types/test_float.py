...
class FloatTest(unittest.TestCase):
    ...
    def test_min_only(self):
        prop = flapi_schema.types.Float(min_value=0)
    ...
    def test_max_only(self):
        prop = flapi_schema.types.Float(max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_min_and_max(self):
        prop = flapi_schema.types.Float(min_value=0, max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_nullable_allows_null(self):
        prop = flapi_schema.types.Float(nullable=True)
        self.assertIsNone(prop(None))

    def test_nullable_raises_error(self):
        prop = flapi_schema.types.Float(nullable=False)
        self.assertRaises(flapi_schema.errors.SchemaValidationError, prop, None)

    def test_default_is_none(self):
        prop = flapi_schema.types.Float(default=None)
        self.assertIsNone(prop(None))

    def test_default_value(self):
        prop = flapi_schema.types.Float(default=12)
    ...
    def test_default_callable(self):
        prop = flapi_schema.types.Float(default=lambda: 12)
        self.assertEqual(prop(None), 12)

    def test_wrong_type(self):
        prop = flapi_schema.types.Float(callback=None)
        self.assertRaises(flapi_schema.errors.SchemaValidationError, prop, "nope")

    def test_callback(self):
        prop = flapi_schema.types.Float(callback=lambda v: v * 2)
