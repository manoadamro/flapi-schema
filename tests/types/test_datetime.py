...
class DateTimeTest(unittest.TestCase):
    ...
    def test_from_timestamp(self):
        prop = flapi_schema.types.DateTime()
        self.assertEqual(
    ...
    def test_min_only(self):
        prop = flapi_schema.types.DateTime(min_value=self.epoc)
        self.assertEqual(prop(self.millenium), self.millenium)

    def test_min_only_out_of_range(self):
        prop = flapi_schema.types.DateTime(min_value=self.millenium)
        self.assertRaises(flapi_schema.errors.SchemaValidationError, prop, self.epoc)

    def test_max_only(self):
        prop = flapi_schema.types.DateTime(max_value=self.millenium05)
        self.assertEqual(prop(self.millenium), self.millenium)

    def test_max_only_out_of_range(self):
        prop = flapi_schema.types.DateTime(max_value=self.millenium)
        self.assertRaises(
    ...
    def test_nullable_allows_null(self):
        prop = flapi_schema.types.DateTime(nullable=True)
        self.assertIsNone(prop(None))

    def test_nullable_raises_error(self):
        prop = flapi_schema.types.DateTime(nullable=False)
        self.assertRaises(flapi_schema.errors.SchemaValidationError, prop, None)

    def test_default_is_none(self):
        prop = flapi_schema.types.DateTime(default=None)
        self.assertIsNone(prop(None))

    def test_default_value(self):
        prop = flapi_schema.types.DateTime(default=self.epoc)
    ...
    def test_default_callable(self):
        prop = flapi_schema.types.DateTime(default=lambda: self.millenium)
        self.assertEqual(prop(None), self.millenium)

    def test_wrong_type(self):
        prop = flapi_schema.types.DateTime(callback=None)
